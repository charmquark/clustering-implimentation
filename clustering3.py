...
"""
clustering3.py by Brian Charous and Yawen Chen
An implementation of k-means clustering technique(part3)

PART3 uses normalize_row to normalize data without calculating the log.
PAER3 also normalize the center using spheral k-means method by Dhillon and Modha 
...
def normalize_center(center):
    ...
    new = 0
    for i in range(len(center.location)):
        magnitude += center.location[i]**2
    magnitude = math.sqrt(magnitude)
    for d in range(len(center.location)):
        center.location[d] = center.location[d]/magnitude
    for t in range(len(center.location)):
         new  +=  center.location[t]**2
    new = math.sqrt(new)
    return center
...
def main():
    ...
    for center in clusters:
        magnitude = 0
        for point in center.location:
            magnitude+= point**2
        magnitude = math.sqrt(magnitude)
        print center 
        print "with the magnitude of the center to be:{0}\n".format(magnitude)
